...
;Variables
  ...
  !define PRODUCT_NAME "cpwallet-cmm"
  !define PRODUCT_WEB_SITE "https://github.com/cmmpay/cpwallet-cmm"
  !define PRODUCT_PUBLISHER "Open Remit Ltd"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/cpwallet-cmm-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\cpwallet-cmm.ico"
...
Section
  ...
  File /r "dist\cpwallet-cmm\*.*"
  File "icons\cpwallet-cmm.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe" "" "$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe" 0


  ;Links commercium: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\commercium" "" "URL:commercium Protocol"
  WriteRegStr HKCU "Software\Classes\commercium" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\commercium" "DefaultIcon" "$\"$INSTDIR\cpwallet-cmm.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\commercium\shell\open\command" "" "$\"$INSTDIR\cpwallet-cmm-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\cpwallet-cmm.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\commercium"
