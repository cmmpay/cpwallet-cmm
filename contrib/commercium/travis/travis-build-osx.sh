...
cp contrib/commercium/osx.spec .
cp contrib/commercium/pyi_runtimehook.py .
cp contrib/commercium/pyi_tctl_runtimehook.py .

pyinstaller \
    -y \
    --name cpwallet-cmm-$ELECTRUM_commercium_VERSION.bin \
    osx.spec

sudo hdiutil create -fs HFS+ -volname "cpwallet-cmm" \
    -srcfolder dist/cpwallet-cmm.app \
    dist/cpwallet-cmm-$ELECTRUM_commercium_VERSION-macosx.dmg
