...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile commercium-electrum \
        -keystore ~/.jks/keystore \
        Electrum_commercium-3.0.6.1-release-unsigned.apk \
        electrum.z.cash
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_commercium-3.0.6.1-release-unsigned.apk \
        Electrum_commercium-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^cpwallet-cmm-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'cpwallet-cmm-{version}'
PPA_SOURCE_NAME = 'cpwallet-cmm'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_commercium-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Electrum_commercium-{version}-release.apk'
