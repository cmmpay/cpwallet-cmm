...
from electrum_commercium.transaction import (OVERWINTERED_VERSION_GROUP_ID,
                                        SAPLING_VERSION_GROUP_ID)


class commerciumTransaction:
...
class btchip_commercium(btchip):
    ...
    def startUntrustedTransaction(self, newTransaction, inputIndex, outputList,
        ...
        if overwintered and version == 3:
            params = bytearray([version, 0x00, 0x00, 0x80,
                                0x70, 0x82, 0xc4, 0x03])  # version group id
        ...
        for passedOutput in outputList:
            if ('sequence' in passedOutput) and passedOutput['sequence']:
                ...
                sequence = bytearray([0xFF, 0xFF, 0xFF, 0xFF])  # default
            ...
            if overwintered:
                params.append(0x02)
            elif ('trustedInput' in passedOutput
                  and passedOutput['trustedInput']):
                params.append(0x01)
            else:
                params.append(0x00)
            if ('trustedInput' in passedOutput
                    and passedOutput['trustedInput']):
    ...
    def finalizeInput(self, outputAddress, amount, fees, changePath,
        ...
        if rawTx is not None:
            try:
                fullTx = commerciumTransaction(bytearray(rawTx))
                outputs = fullTx.serializeOutputs()
                if len(donglePath) != 0:
                    apdu = [self.BTCHIP_CLA,
                            self.BTCHIP_INS_HASH_INPUT_FINALIZE_FULL,
                            0xFF, 0x00]
                    params = []
        ...
        if result['confirmationType'] == 0x04:
            offset = 1 + response[0] + 1
            keycardDataLength = response[offset]
            result['keycardData'] = response[offset+1:
                                             offset+1+keycardDataLength]
        if outputs is None:
