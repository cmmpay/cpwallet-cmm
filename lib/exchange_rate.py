...
class ExchangeBase(PrintError):
    ...
    def get_json(self, site, get_string):
        ...
        response = requests.request('GET', url, headers={'User-Agent' : 'cpwallet-cmm'}, timeout=10)
        return response.json()

    def get_csv(self, site, get_string):
        url = ''.join(['https://', site, get_string])
        response = requests.request('GET', url, headers={'User-Agent' : 'cpwallet-cmm'})
...
class Poloniex(ExchangeBase):
    def get_rates(self, ccy):
        ...
        quote_currencies = {}
        commercium_ticker = json.get('BTC_ZEC')
        quote_currencies['BTC'] = Decimal(commercium_ticker['last'])
        return quote_currencies
