...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("cpwallet-cmm directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(
                _('cpwallet-cmm datadir does not exist. Was it deleted while running?') + '\n' +
...
def read_user_config(path):
    """Parse and store the user config settings in cpwallet-cmm.conf into user_config[]."""
    if not path:
        return {}
