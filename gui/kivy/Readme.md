...
The Kivy GUI is used with cpwallet-cmm on Android devices. To generate an APK file, follow these instructions.

## 1. Install python-for-android (p4a)
p4a is used to package cpwallet-cmm, Python, SDL and a bootstrap Java app into an APK file. 
We patched p4a to add some functionality we need for cpwallet-cmm. Until those changes are
...
In the `gui/kivy` directory of cpwallet-cmm, run `make theming`.

## 6. Download cpwallet-cmm dependencies
